...
<header x-data="mobileNav" @scroll.window="scrolled = window.scrollY > 8" @keydown.escape.window="if (open) close()"
    ...
    <div class="container-prose">
        <div class="flex h-16 items-center justify-between sm:h-20">
            ...
            <a href="{{ route('home') }}" class="group flex items-center gap-3">
                <span
                    class="flex h-10 w-10 items-center justify-center rounded-xl bg-brand-600 text-white shadow-soft transition-transform group-hover:scale-105">
                    <svg class="h-6 w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                        stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                        {{-- Heart + cross mark --}}
                        <path
                            d="M12 21s-7-4.5-9.5-9.5C1 7.5 4 4 7.5 4c1.8 0 3.4 1 4.5 2.5C13.1 5 14.7 4 16.5 4 20 4 23 7.5 21.5 11.5 19 16.5 12 21 12 21z"
                ...
                <span class="flex flex-col leading-tight">
                    ...
                    <span class="text-[10px] font-medium uppercase tracking-widest text-brand-700">Compassionate Heart
                        of Jesus</span>
                </span>
            </a>
            ...
            <nav class="hidden lg:flex items-center gap-1" aria-label="Primary">
                @foreach ($nav as $item)
                    @if (isset($item['children']))
                        <div class="group relative" x-data="{ open: false }" @mouseenter="open = true"
                            ...
                            <a href="{{ $item['route'] }}"
                                ...
                                :class="open ? 'bg-brand-50 text-brand-800' : ''" @click="open = true"
                                ...
                                <div class="overflow-hidden rounded-xl bg-white shadow-lifted ring-1 ring-stone-200">
                                    <ul class="py-1.5">
                                        @foreach ($item['children'] as $child)
                                            <li>
                                                <a href="{{ $child['route'] }}"
                                                    class="block px-4 py-2.5 text-sm text-stone-700 hover:bg-brand-50 hover:text-brand-800 transition-colors">
                                                    {{ $child['label'] }}
                                                </a>
                                            </li>
                                        @endforeach
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('get-involved.donate') }}"
                    class="hidden sm:inline-flex items-center gap-1.5 rounded-lg bg-brand-600 px-4 py-2.5 text-sm font-semibold text-white shadow-soft transition-all hover:bg-brand-700 hover:shadow-lifted">
